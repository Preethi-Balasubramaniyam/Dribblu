...
export default function BlogDetailPage({ params }: Props) {
  ...
  const others = related.length < 2
    ? [...related, ...blogs.filter((b) => b.slug !== blog.slug && !related.includes(b)).slice(0, 2 - related.length)]
    : related
  ...
  return (
    <>
      {/* ── Hero banner ── */}
      <div
        className={`bg-gradient-to-br ${blog.coverColor} pt-36 pb-16 px-6`}
      >
          ...
          <div className="max-w-7xl mx-auto px-6">
            ...
            <div className="grid gap-8 sm:grid-cols-2">
              {others.map((b) => (
                ...
                >
                  <div className={`bg-gradient-to-br ${b.coverColor} h-40 flex items-end p-5`}>
                    <span
                      className={`text-xs font-semibold px-3 py-1 rounded-full ${
